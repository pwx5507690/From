define(['jQuery', 'modal', 'app'], function (jQuery, modal, app) {
    ...
    window.loadingTip = function (tip) {
        tip = tip || "正在载入...";
        jQuery("#loadingTip").html(tip);
    };
    window.loading = function (isShow, action) {
        if (isShow && loadStats == false) {
            loadStats = true;
            window.loadingTip(action);
            modal.showLoading();
        }
        else
            setTimeout(function () {
                loadStats = false;
                modal.closeLoading(action);
            }, 500);
        return this;
    };
