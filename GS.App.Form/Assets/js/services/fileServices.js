define(['jQuery', 'baseServices', 'app'], function (jQuery, baseServices, app) {
	var departmentServices = function () {
		...
		this.existByCode = function (code, isArgsCode, sendCall) {
			var api = "department/exist/{0}/isArgsCode/{1}".format(code, isArgsCode);
		...
		this.existByName = function (name, isArgsName, sendCall) {
			var api = "department/exist/{0}/isArgsName/{1}".format(name, isArgsName);
			this.send(api, this.method.get, null, this.dataType.text, sendCall, function (p, a, b, c) {
