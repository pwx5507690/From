	...
	*/
	...
	var display_icons = function (timer) {
		$('#fileinfo').find('td:first-child').each(function () {
			...
			var treenode = $('#filetree').find('a[data-type="' + type + '"][data-path="' + path + '"]').parent();
	...
	var getName = function (path) {
		var a = $("a[data-path='" + path + "']")
		var text = a.html();
		var parent = $("a[data-path='" + a.attr("data-parent") + "']");
		while (parent.length > 0) {
			text = parent.html() + "\\" + text;
			parent = $("a[data-path='" + parent.attr("data-parent") + "']");
		}
		return text;
	}
	...
	var setUploader = function (path) {
		...
		$('#newfolder').unbind().click(function () {
			...
			var msg = lg.prompt_foldername + '  <input id="fname"  name="fname" style="height:28px; font-size:12px; margin-bottom:10px;"  placeholder="请输入文件名" class="am-form-field"  value="' + foldername + '" />';

			var getFolderName = function (v, m) {
				if (v != 1) return false;
				var fname = m.find("#fname").val();
				...
				if (fname != '') {
					...
					$.getJSON(fileConnector + '?mode=AddFolder&path=' + $('#currentpath').val() + '&config=' + userconfig + '&name=' + encodeURI(foldername) + '&time=' + d.getMilliseconds(), function (result) {
						...
						if (result['Code'] == 0) {
							...
							addFolder(result['Parent'], result['Name'], result['Id']);

							getFolderInfo(result['Id']);
							//$('#filetree').find('a[data-path="' + result['Id'] + '"]')
							//.click().click();
			...
			$.prompt(msg, {
				...
				buttons: btns,
				btnClass: "am-btn am-btn-secondary am-btn-xs"
			});
		});
	};
	...
	var renameItem = function (data) {
		...
		var msg = lg.new_filename + '  <input style="height:28px; font-size:12px; margin-bottom:10px;" id="rname" name="rname" class="am-form-field"  placeholder="请输文件名称" type="text" value="' + fileName + '" />';
		...
		var getNewName = function (v, m) {
			...
			if (rname != '') {
				...
				var fileType = data['File Type'].toUpperCase();
				var connectString = fileConnector + '?mode=Rename&fileType=' + fileType + '&path=' + encodeURIComponent(data['Path']) + '&newName=' + encodeURIComponent(givenName) + '&config=' + userconfig;
	...
	var deleteItem = function (data) {
		...
		var doDelete = function (v, m) {
			if (v != 1) return false;
			var d = new Date(); // to prevent IE cache issues
			var connectString = fileConnector + '?mode=Delete&fileType=' + data["File Type"] + '&path=' + encodeURIComponent(data['Path']) + '&time=' + d.getMilliseconds() + '&config=' + userconfig;
			...
			$.ajax({
				...
				success: function (result) {
					...
					if (result['Code'] == 0) {
						...
						$('#uploader h1')
							.text(lg.current_folder + displayPath(rootpath))
							.attr("title", displayPath(rootpath, false)).attr('data-path', rootpath);
						isDeleted = true;
	...
	var addFolder = function (parent, name, id) {
		var newNode = null;
		if (id) {
			newNode = '<li class="directory collapsed directory-locked"><a class="cap_select cap_rename cap_delete" data-type="DIR" data-parent="' + parent + '" data-path="' + id + '" href="#">' + name + '</a><ul class="jqueryFileTree" style="display: block;"></ul></li>';
		} else {
			newNode = '<li class="directory collapsed directory-locked"><a class="cap_select cap_rename cap_delete" data-type="DIR" data-path="' + parent + name + '/" href="#">' + name + '</a><ul class="jqueryFileTree" style="display: block;"></ul></li>';
		}
		var parentNode = $('#filetree').find('a[data-path="' + parent + '"]');
		if (+parent !== -1) {
			parentNode.next('ul').prepend(newNode);

		} else {
			$('#filetree ul.jqueryFileTree').prepend(newNode);

		}
		var a = $('#filetree').find('li a[data-path="' + id + '"]');
		if (a.attr("data-bind-click") != "true") {
			bindTree(null, a);
			$('#filetree').find('li a[data-path="' + id + '"]')
					//.attr('class', 'cap_select cap_rename cap_delete')
					.click(function () {
						getFolderInfo(id);
					}).each(function () {
						$(this).contextMenu(
	...
	var getDetailView = function (path, type) {
		if (type != null && type.toLocaleUpperCase() !== "DIR") {
			return;
		}
		getFolderInfo(path);
	...
	function getContextMenuOptions(elem) {
		...
		var optionsID = "";
		try {
			optionsID = elem.attr('class').toString().replace(/ /g, '_');
		} catch (e) {
			optionsID = elem;
			console.log(e);
		}
	...
	var setMenus = function (action, path, type) {
		...
		var d = new Date(); // to prevent IE cache issues
		$.getJSON(fileConnector + '?mode=GetInfo&fileType=' + fileType + '&path=' + encodeURIComponent(path) + '&config=' + userconfig + '&time=' + d.getMilliseconds(), function (data) {
			if ($('#fileinfo').data('view') == 'grid') {
	...
	var getFileInfo = function (file) {
		...
		var template = '<div id="preview"><img /><div id="main-title"><h1></h1><div id="tools"></div></div><table class="am-table  am-table-striped tpl-table-black am-table-bordered" style="font-size:12px;width:50%;margin:0 auto" class="am-table"><tbody></tbody></table></div>';
		template += '<form id="toolbar" class="am-form-group"><div class="am-btn-toolbar"><div class="am-btn-group am-form-group-sm" style="float:none">';
		template += '<button class="am-btn am-btn-secondary"  id="parentfolder">' + lg.parentfolder + '</button>';
		...
		if ($.inArray('download', capabilities) != -1) template += '<button id="download" class="am-btn am-btn-secondary" name="download" type="button" value="Download">' + lg.download + '</button>';
		if ($.inArray('rename', capabilities) != -1 && config.options.browseOnly != true) template += '<button id="rename" class="am-btn am-btn-secondary" name="rename" type="button" value="Rename">' + lg.rename + '</button>';
		if ($.inArray('move', capabilities) != -1 && config.options.browseOnly != true) template += '<button id="move" name="move" class="am-btn am-btn-secondary" type="button" value="Move">' + lg.move + '</button>';
		if ($.inArray('delete', capabilities) != -1 && config.options.browseOnly != true) template += '<button id="delete" class="am-btn am-btn-danger" name="delete" type="button" value="Delete">' + lg.del + '</button>';
		if ($.inArray('replace', capabilities) != -1 && config.options.browseOnly != true) {
			template += '<button id="replace" class="am-btn am-btn-success " name="replace" type="button" value="Replace">' + lg.replace + '</button>';
		...
		$.getJSON(fileConnector + '?mode=GetInfo&path=' + encodeURIComponent(file) + '&config=' + userconfig + '&time=' + d.getMilliseconds(), function (data) {
			...
			if (data['Code'] == 0) {
				...
				var properties = '';
				//<tr>
				//			<td style="vertical-align:middle">数据名称 Name</td> properties += '<dt>' + lg.created + '</dt><dd>' + data['Properties']['Date Created'] + '</dd>'; '<dt>' + lg.modified + '</dt><dd>' + data['Properties']['Date Modified'] + '</dd>';
				//'<dt>' + lg.size + '</dt><dd>' + formatBytes(data['Properties']['Size']) + '</dd>';
				if (data['Properties']['Width'] && data['Properties']['Width'] != '') properties += '<tr><td style="vertical-align:middle;width:50%;text-align:right">' + lg.dimensions + '</td><td style="text-align:left">' + data['Properties']['Width'] + 'x' + data['Properties']['Height'] + '</td></tr>';
				if (data['Properties']['Date Created'] && data['Properties']['Date Created'] != '') properties += '<tr><td style="vertical-align:middle;width:50%;text-align:right">' + lg.created + '</td><td style="text-align:left">' + data['Properties']['Date Created'] + '</td></tr>';
				if (data['Properties']['Date Modified'] && data['Properties']['Date Modified'] != '') properties += '<tr><td style="vertical-align:middle;width:50%;text-align:right">' + lg.modified + '</td><td style="text-align:left">' + data['Properties']['Date Modified'] + '</td></tr>';
				if (data['Properties']['Size'] || parseInt(data['Properties']['Size']) == 0) properties += '<tr><td style="vertical-align:middle;width:50%;text-align:right">' + lg.size + '</td><td style="text-align:left">' + formatBytes(data['Properties']['Size']) + '</td></tr>';
				$('#fileinfo').find('tbody').html(properties);
	...
	var getFolderInfo = function (path) {
		...
		var d = new Date(); // to prevent IE cache issues
		var url = fileConnector + '?path=' + encodeURIComponent(path) + '&config=' + userconfig + '&mode=GetFolderInfo&selectedType=' + selectedType + '&showThumbs=' + config.options.showThumbs + '&time=' + d.getMilliseconds();
		if ($.urlParam('type')) url += '&type=' + $.urlParam('type');
		$.getJSON(url, function (data) {
			if (!data || data.length == 0) {
				$("#fileinfo").empty();
				return $('#loading-wrap').fadeOut(800);
			}
			var result = '';
			...
			if (data) {
				...
				if ($('#fileinfo').data('view') == 'grid') {
					...
					for (key in data) {
						...
						var input = "";
						if (data[key]["File Type"].toUpperCase() != "DIR") {
							if (selectedType == "checkbox") {
								input = '<input data-input=' + data[key]['Path'] + ' type="checkbox" />&nbsp';
							} else if (selectedType == "radio") {
								input = '<input name="attach" data-input=' + data[key]['Path'] + ' type="radio" />&nbsp';
							}
						}
						
						result += '<li title=' + data[key]['Filename'] + ' class="' + cap_classes + '"' + title + '"><div class="clip"><img data-type="' + data[key]["File Type"] + '" src="' + data[key]['Preview'] + '" width="' + scaledWidth + '" alt="' + data[key]['Path'] + '" data-path="' + data[key]['Path'] + '" /></div><p><label class="am-checkbox-inline">' + input + data[key]['Filename'] + '</label></p>';
					...
					result += '<table id="contents" style="margin-left:5px;" class="list">';
					...
					for (key in data) {
						...
						result += '<td data-type="' + data[key]['File Type'] + '" data-path="' + data[key]['Path'] + '"' + title + '">' + data[key]['Filename'] + '</td>';
			...
			if ($('#fileinfo').data('view') == 'grid') {
				$('#fileinfo').find('#contents li').click(function () {
					...
					$(this).contextMenu(
						...
						function (action, el, pos) {
							...
							var type = $(el).find('img').attr('data-type');
							setMenus(action, path, type);
						}
					);
				});
				...
				$('#fileinfo tbody tr').click(function () {
					...
					var type = $('td:first-child', this).attr('data-type');
					if (config.options.quickSelect && data[path]['File Type'] != 'dir' && $(this).hasClass('cap_select')) {
						selectItem(data[path]);
					} else {
						getDetailView(path, type);
					...
					$(this).contextMenu(
						...
						function (action, el, pos) {
							...
							var type = $('td:first-child', el).attr('data-type');
							setMenus(action, path, type);
						}
					);
				});
	...
	var populateFileTree = function (path, callback) {
		var d = new Date(); // to prevent IE cache issues
		var url = fileConnector + '?path=' + encodeURIComponent(path) + '&config=' + userconfig + '&mode=GetFolderInfo&showThumbs=' + config.options.showThumbs + '&time=' + d.getMilliseconds();
		if ($.urlParam('type')) url += '&type=' + $.urlParam('type');
			...
			if (data) {
				...
				for (key in data) {
					...
					if (data[key]['File Type'] == 'dir') {
						...
						result += "<li class=\"directory collapsed" + extraclass + "\"><a data-type='" + data[key]['File Type'] + "' href=\"#\" class=\"" + cap_classes + "\" data-parent='" + data[key]['Parent'] + "' data-path=\"" + data[key]['Path'] + "\">" + data[key]['Filename'] + "</a></li>";
					} else {
						if (config.options.listFiles) {
							var extraclass = data[key]['Protected'] == 0 ? '' : ' file-locked';
							result += "<li class=\"file ext_" + data[key]['File Type'].toLowerCase() + extraclass + "\"><a data-type='" + data[key]['File Type'] + "'  href=\"#\" class=\"" + cap_classes + "\" data-path=\"" + data[key]['Path'] + "\">" + data[key]['Filename'] + "</a></li>";
						}
					}
				}
				result += "</ul>";
	...
	$(function () {
		...
		$('#home').click(function () {
			...
			$('.jqueryFileTree').find('[class="directory directory-locked"]').find('UL').slideUp({
				duration: ol.collapseSpeed,
				easing: ol.collapseEasing
			});
			$('.jqueryFileTree').find('[class="directory directory-locked"]').removeClass('expanded').addClass('collapsed');
		...
		$('#level-up').click(function () {
			...
			if (+cpath != +fileRoot) {
				...
				var p = $("a[data-path='" + cpath + "']");
				if (p.length == 0) {
					return;
				}
				var parent = p.attr("data-parent");
				...
				if (+parent == -1) {
					...
					p.parent().removeClass('expanded').addClass('collapsed');
				}
			}
		});
		...
		if (config.upload.multiple) {
			...
			$("#newfolder")
			.before('<button id="upload" name="upload" type="button" value="Upload" class="am-btn am-btn-success"><i class="am-icon-upload"></i><span>' + lg.upload + '</span></button> ');

			$('#upload').unbind().click(function () {
				// we create prompt
				var msg = "";
				if (selectedType) {
					msg = '<div id="dropzone-container"><div id="multiple-uploads" class="dropzone"></div>';
				}
				else {
					msg = '<div id="dropzone-container"><h2>' + lg.current_folder + $('#uploader h1').attr('title') + '</h2><div id="multiple-uploads" class="dropzone"></div>';
				}
				...
				msg += '<button style="margin-left:10px;" class="am-btn am-btn-secondary am-btn-xs"  id="process-upload">' + lg.upload + '</button></div>';
				...
				$.prompt(msg, {
					buttons: btns,
					btnClass: "am-btn am-btn-danger am-btn-xs"
				});
				$("div#multiple-uploads").dropzone({
					...
					success: function (file, response) {
						...
						var data;
						if (selectedType) {
							$('#uploadresponseType').empty().html(response);
							data = jQuery.parseJSON($('#uploadresponseType').find('textarea').text());
						} else {
							$('#uploadresponse').empty().html(response);
							data = jQuery.parseJSON($('#uploadresponse').find('textarea').text());
						}
	...
	if (selectedType) {
		$("#selected").on("click", function () {
			...
			$("[data-input]").each(function () {
				if (this.checked)
					select.push($(this).attr("data-input"));
			});
			if (select.length == 0) {
				var btns = {};
				btns["确定"] = true;
